...
class Robot_System:
    ...
    def get_qrcode_xyz(self, aruco_id_need):
        """
        获取指定Aruco标签的XYZ坐标。
...
if __name__ == "__main__":
    ...
    # 3. 执行单次检测
    print("开始二维码识别测试...")
    x, y, z = robot_system.get_qrcode_xyz(test_id)
    
    # 4. 输出结果
    if x is not None:
        print(f"✅ 成功检测到ID {test_id}：位置(X,Y,Z) = ({x:.1f}, {y:.1f}, {z:.1f}) mm")
    else:
        print(f"❌ 未检测到ID {test_id}，请检查：")
        print("   - 相机是否正常连接")
        print("   - 二维码是否在视野内")
        print("   - 环境光照是否充足")
        print("   - 配置文件arucotag.yaml中的标签尺寸是否正确")
