...
while True:
    ...
    # YoloV8 实例分割
    result = model.segment(img, draw_mask=True, draw_box=True, draw_label=True)
    canvas, class_id_list, obj_mask_list, xyxy_list, conf_list = result 
    # 结束计时
    t_end = time.time() 
    # 添加fps显示
    ratio = 0.1
    fps = (1-ratio)*fps + ratio*(1.0/(t_end - t_start))
    cv2.putText(canvas, text=f"FPS: {fps:.2f}".format(), org=(50, 50), \
